...
def render_messages():
    """Render messages page for assistant manager."""
    ...
    if st.session_state.delete_message_id:
        if delete_message(st.session_state.delete_message_id, "asst_manager", st.session_state.user_id):
    ...
    # Filter to get only general employees (assistant managers can only message general employees)
        ...
        if company_messages:
            for message in company_messages:
                render_message_card(
                    message=message,
                    sender_info="Company",
                    receiver_info="You (Assistant Manager)",
                    can_delete=False
                )
                ...
                with st.form("reply_company_form"):
                    ...
                    if submit_button:
                        if not message_text:
                            ...
                            message_id = send_message("asst_manager", st.session_state.user_id, "company", st.session_state.company_id, message_text, attachment_link)
    ...
    with tab2:
            ...
            with st.form("send_employee_form"):
                ...
                if submit_employee:
                    if not employee:
                        ...
                        message_id = send_message("asst_manager", st.session_state.user_id, "employee", employee_id, employee_message, employee_attachment)
        ...
        with col2:
            ...
            sent_messages = get_messages(sender_type="asst_manager", sender_id=st.session_state.user_id)
            
            # Get received messages from employees
            received_messages = get_messages(receiver_type="asst_manager", receiver_id=st.session_state.user_id, sender_type="employee")
            ...
            if sent_messages or received_messages:
                ...
                for message in all_messages[:10]:  # Show only latest 10 messages
                    ...
                    if sender_type == "asst_manager" and sender_id == st.session_state.user_id:
                        # Sent by assistant manager (self)
                        employee_id = receiver_id
                        employee_name = next((e[1] for e in general_employees if e[0] == employee_id), f"Employee {employee_id}")
                        
                        render_message_card(
                            message=message,
                            sender_info="You (Assistant Manager)",
